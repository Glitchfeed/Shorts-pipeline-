...
async function triggerAutoGenerate(channel) {
  ...
  createJob(jobId);
  updateJob(jobId, { channel: channel, status: 'running', step: 'Fetching content for ' + channel + '...', progress: 2 });
  ...
  try {
    ...
    updateJob(jobId, { step: 'Script ready', progress: 10, scriptText: script, title: title });
...
function getSchedulerState() {
  return Object.assign({}, schedulerState, { nextRuns: getNextRuns() });
}
