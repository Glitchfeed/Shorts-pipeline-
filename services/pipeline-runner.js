...
async function runScriptPipeline(jobId, script, channel, tempDir) {
  ...
  updateJob(jobId, { step: '🖼️ Generating Pixar 3D scenes...', progress: 37 });
  ...
  for (let i = 0; i < visualPrompts.length; i++) {
    ...
    updateJob(jobId, { step: `🖼️ Generating scene ${i+1} of ${visualPrompts.length}...`, progress: 37 + Math.round((i/visualPrompts.length)*18) });
  ...
  updateJob(jobId, { step: '🎙️ Generating voiceover...', progress: 77 });
...
async function tryYouTubeUpload(jobId, videoPath, title, channel) {
  try {
    ...
    if (!isConnected(channel)) {
      console.log(`No YouTube tokens for ${channel}, skipping upload`);
      return false;
    }
